    ...
    def process_html_to_sentences(self, html_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def llm_purify(text):
            try:
                ...
                llm_manager = LLMInference()

                # Generate unique IDs
                llm_task_id = str(uuid.uuid4())
                task_id = str(uuid.uuid4())
                
                # Create status document in MongoDB
                llm_manager.create_status(
                    qid="text_purification",
                    task_id=task_id,
                    llm_task_id=llm_task_id,
                    infer_type="text_purification"
                )
                
                # Create inference document with the text to be processed
                llm_manager.create_inference(
                    llm_task_id=llm_task_id,
                    input_data=str(text)
                )
                
                # Wait for the background LLM process to complete
                max_wait_time = 600  # 10 minutes timeout
                start_time = time.time()
                
                while True:
                    # Check current status in MongoDB
                    current_status = llm_manager.status_collection.find_one(
                        {"llm_task_id": llm_task_id}
                    )
                    
                    if current_status["llm_status"] == "completed":
                        # Retrieve the processed result
                        result = llm_manager.inference_collection.find_one(
                            {"llm_task_id": llm_task_id}
                        )
                    ...
                    elif current_status["llm_status"] in ["failed", "error"]:
                        return text
        ...
        valid_html_df['html2text'] = valid_html_df['html2text'].apply(lambda x: llm_purify(x))
